...
program.command("push").option("--dry-run", "build and report; no encrypt, no rsync").action(async (opts: { dryRun?: boolean }) => {
    ...
    const config = JSON.parse(await readFile(path.join(rootDir, "codex-sync.config.json"), "utf-8")) as Config;
    ...
    const passphrase = process.env.CODEX_SYNC_PASSPHRASE;
    if (!passphrase) throw new Error("CODEX_SYNC_PASSPHRASE is not set (put it in .env)");
    ...
    console.log(`Pushed manifest ${result.payload.manifestHash} to ${config.remote.host}:${config.remote.dataPath}/codex-sync/`);
});
